...
```json
"usingComponents": {
  "fbm-uploader": "fbmui-weapp/uploader/index"
}
```
...
```html
<fbm-uploader file-list="{{ fileList }}" bind:after-read="afterRead" />
```
...
```html
<fbm-uploader file-list="{{ fileList }}" />
```
...
```html
<fbm-uploader file-list="{{ fileList }}" deletable="{{ true }}" />
```
