...
```json
"usingComponents": {
  "fbm-slider": "fbmui-weapp/slider/index"
}
```
...
```html
<fbm-slider value="50" bind:change="onChange" />
```
...
```html
<fbm-slider value="{{ 10, 50 }}" range @change="onChange" />
```
...
```html
<fbm-slider min="-50" max="50" />
```

### 禁用

```html
<fbm-slider value="50" disabled />
```

### 指定步长

```html
<fbm-slider value="50" step="10" />
```

### 自定义样式

```html
<fbm-slider value="50" bar-height="4px" active-color="#ee0a24" />
```

### 自定义按钮

```html
<fbm-slider value="{{ currentValue }}" use-button-slot bind:drag="onDrag">
...
<view style="height: 150px;">
  <fbm-slider value="50" vertical bind:change="onChange" />
  <fbm-slider
    value="{{ [10, 50] }}"
    range
    vertical
