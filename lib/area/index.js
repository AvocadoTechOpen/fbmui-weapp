...
(0, component_1.VantComponent)({
    ...
    props: {...shared_1.pickerProps, showToolbar: {
            type: Boolean,
            value: true,
        }, value: {
            type: String,
    ...
    methods: {
        getPicker () {
            if (this.picker == null) {
                this.picker = this.selectComponent('.fbm-area__picker');
            }
            return this.picker;
        },
        onCancel (event) {
        ...
        onChange (event) {
            const _this = this;
            let _a;
            const _b = event.detail; const {index} = _b; const {picker} = _b; const {value} = _b;
