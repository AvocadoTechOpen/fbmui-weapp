...
```json
"usingComponents": {
  "fbm-area": "fbmui-weapp/area/index"
}
```
...
Fbmui 官方提供了一份默认的省市区数据，可以通过 [@vant/area-data](https://github.com/AvocadoTechOpen/vant/tree/dev/packages/vant-area-data) 引入。
...
```html
<fbm-area area-list="{{ areaList }}" value="110101" />
```
...
```html
<fbm-area area-list="{{ areaList }}" columns-num="{{ 2 }}" title="标题" />
```
