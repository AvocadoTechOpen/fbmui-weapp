    ...
    methods: {
        ...
        getMultipleDayType (day) {
            ...
            const isSelected = function (date) {
                return currentDate.some((item) => (0, utils_1.compareDay)(item, date) === 0);
            };
            if (isSelected(day)) {
        ...
        getRangeDayType (day) {
            const _a = this.data; const {currentDate} = _a; const {allowSameDay} = _a;
            if (!Array.isArray(currentDate)) {
                return '';
            }
            const startDay = currentDate[0]; const endDay = currentDate[1];
            if (!startDay) {
                return '';
            }
        ...
        getDayType (day) {
            const _a = this.data; const {type} = _a; const {minDate} = _a; const {maxDate} = _a; const {currentDate} = _a;
