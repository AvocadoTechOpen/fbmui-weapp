...
```json
"usingComponents": {
  "fbm-circle": "fbmui-weapp/circle/index"
}
```
...
```html
<fbm-circle value="{{ value }}" stroke-width="6" text="宽度定制" />
```
...
```html
<fbm-circle value="{{ value }}" color="{{ gradientColor }}" text="渐变色" />
```
...
```html
<fbm-circle value="{{ value }}" size="120" text="大小定制" />
```
