...
(0, component_1.VantComponent)({
    ...
    props: {...shared_1.pickerProps, value: {
            type: null,
    ...
    methods: {
        ...
        getPicker () {
            if (this.picker == null) {
                this.picker = this.selectComponent('.fbm-datetime-picker');
        ...
        getRanges () {
            ...
            const _a = this.getBoundary('max', data.innerValue); const {maxYear} = _a; const {maxDate} = _a; const {maxMonth} = _a; const {maxHour} = _a; const {maxMinute} = _a;
            const _b = this.getBoundary('min', data.innerValue); const {minYear} = _b; const {minDate} = _b; const {minMonth} = _b; const {minHour} = _b; const {minMinute} = _b;
            const result = [
                {
                    type: 'year',
