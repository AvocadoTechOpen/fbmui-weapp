...
```json
"usingComponents": {
  "fbm-count-down": "fbmui-weapp/count-down/index"
}
```
...
```html
<fbm-count-down time="{{ time }}" format="DD 天 HH 时 mm 分 ss 秒" />
```
...
```html
<fbm-count-down millisecond time="{{ time }}" format="HH:mm:ss:SSS" />
```
...
```html
<fbm-count-down use-slot time="{{ time }}" bind:change="onChange">
...
<fbm-grid clickable column-num="3">
  ...
  <fbm-grid-item text="重置" icon="replay" bindclick="reset" />
</fbm-grid>
```
